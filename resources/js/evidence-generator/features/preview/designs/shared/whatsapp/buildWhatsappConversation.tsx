...
import * as standardRules from './conversationReplyRules';
import * as mobile1Rules from './conversationReplyRulesMobile1';
import type { ConversationIntent } from './conversationReplyRules';
import type { WhatsappData, WhatsappBehaviorProfile, WhatsappMessageStatus } from './whatsappTypes';
...
export function buildWhatsappConversation(data: WhatsappData, messageStatus?: WhatsappMessageStatus, behaviorProfile: WhatsappBehaviorProfile = 'standard') {
    const rules = behaviorProfile === 'mobile-1' ? mobile1Rules : standardRules;
    ...
    const statusForMessages: WhatsappMessageStatus = messageStatus ?? (rng() < 0.7 ? 'read' : 'delivered');
    ...
    const activeIntent =
        (data.modoEntrada === 'contactado' || data.modoEntrada === 'informativo') && rules.isConversationIntent(pickedInitialResponse?.tipo)
    ...
    const secondExchangePlan =
        data.modoEntrada === 'contactado' && activeIntent !== null
            ? rules.buildContactadoSecondExchange({
                  intent: activeIntent,
