...
export function buildQuestionAwarePlan({
  ...
  if (intent === "analitico") {
    ...
    return {
      advisorReplyLines: [
        "Entiendo, en estos momentos le paso su simulacion donde podra ver el detalle de monto, cuota, plazo y tasa.",
      ],
      followupLines: simulationLines,
    };
  }
  ...
  if (intent === "evaluador") {
    return {
      advisorReplyLines: [
        "Entiendo, en estos momentos le paso su simulacion de este prestamo preaprobado para que pueda ver lo consultado.",
      ],
      followupLines: simulationLines,
    };
  }
  ...
  if (intent === "practico_logistica") {
    if (/fin de semana|fines de semana|horario|horarios|atienden|atencion|sabado|sÃ¡bado/.test(text)) {
      return {
        advisorReplyLines: [
    ...
    return {
      advisorReplyLines: [
        "Claro. El tramite se realiza en la agencia mas cercana del Banco de la Nacion.",
      ],
    };
  }
  ...
  if (intent === "oportunista") {
    if (
      mode === "informativo" &&
      /puede cambiar el monto o condiciones con el tiempo/.test(text)
    ) {
      return {
        advisorReplyLines: [
          "Claro. El prestamo preaprobado puede mantenerse vigente hasta fin de mes, pero cambia cada mes.",
        ],
      };
    }
    ...
    if (/fecha limite|hasta que dia|hasta cuando|tiempo tengo|vigente|vence/.test(text)) {
      return {
        advisorReplyLines: [
          "Claro. En este caso, el prestamo preaprobado puede mantenerse vigente hasta fin de mes.",
        ],
      };
    }
    ...
    return {
      advisorReplyLines: [
        "Claro. El prestamo preaprobado puede mantenerse vigente hasta fin de mes.",
      ],
    };
  }
...
export function buildContactadoSecondExchange({
  ...
  const profileOptionsByIntent: Record<
    TipoCliente,
    Record<Exclude<ConversationIntent, "desinteresado">, ContactadoSecondExchangePlan[]>
  > = {
    apurado: {
      informacion_general: [
        {
