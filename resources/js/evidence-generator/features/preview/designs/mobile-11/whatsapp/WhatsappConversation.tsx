...
import { getDateKeyFromLocalDateTime, getMobileDayChipTextForDate } from '../../../../../lib/whatsapp/time';
...
import type { WhatsappBehaviorProfile, WhatsappData, WhatsappMessageStatus } from './whatsappTypes';
import type { WhatsappMobileVisualAdapter } from './whatsappVisualAdapter';
...
function linesToSpans(lines: ReactNode[], themeMode: PreviewThemeMode) {
    return lines.map((line, idx) => {
        ...
        return (
            <span key={key} className="leading-[18.75px]">
                {typeof line === 'string' ? renderFormattedLine(line, idx, themeMode) : line}
            </span>
        );
    });
}
...
function formatMobile11TimeValue(value: string): string {
    const trimmedValue = value.trim();
    const match = /^(\d{1,2}):(\d{2})(?:\s*([ap])\.m\.)?$/i.exec(trimmedValue);
    ...
    if (period) {
        if (hours < 1 || hours > 12) return trimmedValue;
        hours = hours % 12 + (period === 'p' ? 12 : 0);
    }

    if (hours < 0 || hours > 23 || minutes < 0 || minutes > 59) return trimmedValue;
...
function normalizeGeneratedMessages(
    ...
    messageStatus: WhatsappMessageStatus | undefined,
): WhatsappConversationMessage[] | null {
    if (!messages) return null;

    return messages.map((msg) => ({
        ...msg,
        time: formatMobile11TimeValue(msg.time),
    ...
    return (
        <div className="h-full w-full overflow-hidden">
            <div className="relative h-full min-h-0 overflow-hidden">
                <visualAdapter.ConversationBackground themeMode={themeMode} />
                    ...
                    <div className="relative min-h-0 flex-1 overflow-hidden">
                                            ...
                                            <visualAdapter.Bubble
                                                ...
                                                quote={
                                                    msg.quote && quoteColors
                                                        ? {
                                                              author: msg.quote.side === 'out' ? 'T̼' : (displayTitle ?? 'Cliente'),
                                                              text: msg.quote.text,
                        ...
                        {showMoreConversationIndicator && (
                            <div className="pointer-events-none absolute right-[12.5px] bottom-[12.5px] z-30">
                                <visualAdapter.MoreConversationIndicator themeMode={themeMode} />
                            </div>
                        )}
                    </div>

                    <div className="shrink-0">
                        <visualAdapter.InputBar themeMode={themeMode} composerAccessory={composerAccessory} composerLayout={composerLayout} />
                    </div>
                </div>
            </div>
        </div>
