...
export function DayChip({ text, themeMode = 'light' }: { text: string; themeMode?: PreviewThemeMode }) {
    ...
    return (
        <div className="sticky top-[2.5px] z-20 my-1.25 flex justify-center">
            <span
                className={[
                    'rounded-[7.5px] px-2.5 py-[2.5px] text-[11.25px] font-medium shadow',
...
export function IncomingBubble({ children }: { children: React.ReactNode }) {
    return (
        <div className="flex justify-start">
            <div className="max-w-[85%] rounded-[20px] rounded-tl-[7.5px] border border-black/5 bg-white px-3.75 py-2.5 text-[15px] text-slate-900 shadow-[0_1.25px_0_rgba(0,0,0,0.04)]">
                {children}
            </div>
        </div>
    );
...
export function OutgoingBubble({ children }: { children: React.ReactNode }) {
    return (
        <div className="flex justify-end">
            <div className="max-w-[85%] rounded-[20px] rounded-tr-[7.5px] border border-black/5 bg-[#d9fdd3] px-3.75 py-2.5 text-[15px] text-slate-900 shadow-[0_1.25px_0_rgba(0,0,0,0.04)]">
                {children}
            </div>
        </div>
    );
...
export function BubbleTitle({ children }: { children: React.ReactNode }) {
    return <div className="mb-1.25 text-[15px] font-semibold">{children}</div>;
}

export function BubbleRow({ k, v }: { k: string; v: string }) {
    const value = v?.trim() ? v : '—';
    return (
        <div className="flex items-baseline justify-between gap-3.75">
        ...
        <Bubble side={side} firstInGroup={firstInGroup} time={time} status={status} themeMode={themeMode}>
            <div className="flex items-center gap-2.5">
                ...
                <div className="min-w-[150px] flex-1">
                    <div className="relative h-3.75 overflow-hidden rounded-full bg-black/10">
                        <div className="absolute top-0 left-0 h-full w-[35%] bg-black/20" />
                    </div>
                    <div className="mt-1.25 text-[12.5px] text-[#667781]">{duration}</div>
                </div>
...
export function EncryptedMessage({ themeMode = 'light' }: { themeMode?: PreviewThemeMode }) {
    const isDark = themeMode === 'dark';
    const encryptedTextSize = 'text-[11px] leading-[12.5px] !font-light';
                    ...
                    <div className="flex justify-center">
                        <div>
                            <div className={`mx-auto flex ${maxWidth} flex-col justify-center ${horizontalPadding}`}>
                                ...
                                <div
                                    className={[
                                        'relative mb-0 box-border inline-block max-w-full flex-none rounded-[9.1875px] px-3.75 pt-1.25 pb-[6.25px] text-center text-[11.5px] leading-[15px] shadow-[0_1.25px_0.625px_rgba(11,20,26,0.13)]',
                                        isDark ? 'bg-[#12181C] text-[#eed294]' : 'bg-[#FFF0D4] text-black/60',
                                    ].join(' ')}
                                >
                                    ...
                                    <div className="absolute top-1/2 order-0 -mt-[16.25px] flex min-h-0 w-[126.25px] min-w-0 shrink grow-0 basis-auto flex-row flex-nowrap items-center justify-start self-auto justify-self-auto px-1.25"></div>
                                </div>
                            </div>
                        </div>
                    </div>
...
export function TempporalMessage({ themeMode = 'light' }: { themeMode?: PreviewThemeMode }) {
    ...
    const horizontalPadding = 'px-10';
    const maxWidth = 'max-w-none';
    const messageTextSize = 'text-[11px] leading-[12.5px]';
                    ...
                    <div className="flex justify-center">
                        <div>
                            <div className={`mx-auto flex ${maxWidth} flex-col justify-center ${horizontalPadding}`}>
                                ...
                                <div
                                    className={[
                                        'relative mb-0 box-border inline-block max-w-full flex-none rounded-[9.1875px] px-3.75 pt-1.25 pb-[6.25px] text-center text-[12.5px] leading-[15px] shadow-[0_1.25px_0.625px_rgba(11,20,26,0.13)]',
                                        isDark ? 'bg-[#12181C] text-[#767C80]' : 'bg-[#fefdfc] text-[#667781]',
                                    ].join(' ')}
                                >
                                    <div className="relative z-10 rounded-[9.1875px]">
            ...
            <div className="relative pb-2.5">
                <div className="flex justify-center">
                    <div className="mx-auto flex w-fit max-w-full flex-col items-center justify-center">
                        <div
                            className={[
                                'relative mb-0 box-border inline-flex w-fit max-w-full flex-none items-center rounded-[9.1875px] px-3.75 pt-1.25 pb-[6.25px] text-center text-[12.5px] leading-[15px] shadow-[0_1.25px_0.625px_rgba(11,20,26,0.13)]',
                                isDark ? 'bg-[#12181C] text-[#767C80]' : 'bg-[#fefdfc] text-[#667781]',
                            ].join(' ')}
                        >
                            <div className="relative z-10 rounded-[9.1875px]">
                                <div className="cursor-pointer">
                                    <span className="inline-flex items-start justify-center gap-[2.5px]">
                                        <span className="inline-block" aria-hidden="true" data-icon="lock-small">
                                            <svg
                                                viewBox="0 0 24 24"
                                                height={11.25}
                                                width={11.25}
