...
function QuotedMessageBox({ quote, themeMode }: { quote: QuotedMessage; themeMode: PreviewThemeMode }) {
    ...
    return (
        <div className={['rounded-[8.75px]', isDark ? 'bg-black/20' : 'bg-black/5'].join(' ')}>
            <div className="mb-[3.75px] flex overflow-hidden rounded-[7.5px]">
                <div
                    className={['w-1.25 shrink-0 rounded-s-[7.5px]', quote.accentColor ? '' : (quote.accentClassName ?? 'bg-[#0063CB]')].join(' ')}
                    style={quote.accentColor ? { backgroundColor: quote.accentColor } : undefined}
                />
                <div className="flex min-w-0 flex-col gap-[7.5px] px-2.5 pt-1.25 pb-2.5">
                    <p
                        className="truncate text-[13.125px] leading-5 font-semibold text-[#0078D7]"
                        style={quote.authorColor ? { color: quote.authorColor } : undefined}
                    >
                        {quote.author}
                    </p>
                    <p className={['line-clamp-2 text-[15px] leading-5', isDark ? 'text-[#878F92]' : 'text-black/60'].join(' ')}>{quote.text}</p>
                </div>
            </div>
        </div>
    );
...
export function WhatsappMobileTextBubble({ side, firstInGroup, time, status, id, quote, themeMode = 'light', children }: WhatsappTextBubbleProps) {
    ...
    return (
        <div id={id} className={`flex px-3.75 ${isOut ? 'justify-end' : 'justify-start'}`}>
            <div className="relative max-w-[87%] flex-none text-[17.75px] leading-[15px] ">
                {firstInGroup && <BubbleTail side={isOut ? 'right' : 'left'} colorClass={tailColor} />}

                <div
                    className={[
                        'relative z-10 rounded-[9.375px]',
                        cornerCut,
                        bubbleBg,
                        bubbleText,
                        'shadow-[0_1.25px_0.625px_rgba(11,20,26,0.13)]',
                    ].join(' ')}
                >
                    <div className="box-border p-2.25 select-text">
                        {quote ? <QuotedMessageBox quote={quote} themeMode={themeMode} /> : null}

                        <div className="relative overflow-hidden ps-[3.75px] pe-[3.75px] break-words whitespace-pre-wrap">
                            <span
                                className="visible text-[14.5px] leading-[15px] font-[450] tracking-[0.00625rem]"
                                style={{ minHeight: '0px' }}
                            >
                                {content}
                            </span>
                            <span>
                                <span aria-hidden="true" className="invisible inline-flex h-0 align-middle text-[0.859375rem] leading-[23.75px]">
                                    {isOut ? <span className="w-5 shrink-0 grow-0" /> : null}
                        ...
                        {(time || (isOut && status)) && (
                            <div className="relative z-10 float-right -mt-3.75 -mb-[6.25px] ps-1.25 pe-0">
                                <div
                                    className={[
                                ...
                                >
                                    {time && (
                                        <span className="inline-block align-top" dir="auto">
                                            <span
                                                className={[
                                                    'inline max-w-full min-w-0 text-[12.5px] leading-5 font-normal wrap-break-word break-all whitespace-pre-line select-text',
                                                    metaText,
                                                ].join(' ')}
                                            >
                                                {time}
                                    ...
                                    {isOut && status && (
                                        <div className="flex justify-end ps-[3.75px]">
