...
export function Mobile11PreviewFooter({ themeMode, variant = 'default' }: { themeMode: PreviewThemeMode; variant?: 'default' | 'whatsapp' | 'sms' }) {
    return <Mobile11SamsungNavigationFooter themeMode={themeMode} variant={variant} />;
}

function Mobile11SamsungNavigationFooter({ themeMode, variant }: { themeMode: PreviewThemeMode; variant: 'default' | 'whatsapp' | 'sms' }) {
    const colors = getSmsColors(themeMode, 'mobile-3');
    const backgroundColor = variant === 'whatsapp' && themeMode === 'dark' ? '#000000' : colors.conversation;

    return (
        <nav className="flex h-[50px] shrink-0 items-center justify-center" style={{ backgroundColor, color: colors.systemNavigationForeground }} aria-label="Navegación del sistema">
