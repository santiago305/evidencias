...
export function Mobile11BatteryIcon({ batteryLevel, charging = false }: { batteryLevel: number; charging?: boolean }) {
    const level = Math.max(0, Math.min(100, Math.round(batteryLevel)));
    const progressWidth = (48 * level) / 100;
    const progressColor = getMobile11BatteryColor(level, '#EDEDED');
    ...
    return (
        ...
        >
            ...
            <text x={numberX} y="15.1" textAnchor="middle" dominantBaseline="middle" fill="#3A3A3A" fontFamily="'Roboto Condensed', 'Roboto', sans-serif" fontSize="22.5"fontWeight="800" letterSpacing="-1.2">
                {level}
            </text>
        </svg>
    );
