...
import { Mobile6ConnectivityIcons } from './components/status-bar/Mobile6ConnectivityIcons';
import { Mobile6WifiIcon } from './components/status-bar/Mobile6WifiIcon';
...
function BatteryIcon({ level }: { level: BatteryLevel }) {
    return <AndroidBatteryIcon level={level} className="h-[16.25px] w-[32.5px] text-current mt-1" />;
}

type Mobile6PreviewHeaderProps = {
...
export function Mobile6PreviewHeader({
    themeMode,
    notificationSeed,
    notificationIds,
    variant = 'default',
    ...
    batteryRenderer,
}: Mobile6PreviewHeaderProps) {
    const isDark = themeMode === 'dark';
    const isWhatsappVariant = variant === 'whatsapp' && isDark;
    const isSmsVariant = variant === 'sms' && isDark;

    // ==================================================
    // Mobile-6 SMS — status bar time
    // ==================================================

    const mobile6SmsTimeClassName = 'text-[14px] leading-[16px] font-[600] tracking-[0px]';
    const mobile6SmsTimeColor = '#E0E0E0';
        ...
        >
            ...
            >
                <div className="flex items-center gap-[7.5px]">
                    <span
                        className={[
                            isSmsVariant ? mobile6SmsTimeClassName : 'text-[16px] leading-none font-semibold tracking-[-0.01em]',
                            !isSmsVariant && (systemHeaderForeground ? '' : isWhatsappVariant ? 'text-white' : isDark ? 'text-white' : 'text-[#5f6368]'),
                        ]
                            .filter(Boolean)
                            .join(' ')}
                        style={isSmsVariant ? { color: mobile6SmsTimeColor } : undefined}
                    >
                        {time}
                    </span>
                ...
                <div className="flex items-center gap-[5px] text-current">
                    ...
                    <Mobile6ConnectivityIcons
                        smsDark={isSmsVariant}
                        wifiIcon={
                            <Mobile6WifiIcon
                                className={[
                                    'h-[18px] w-[22px]',
                                    isSmsVariant ? 'text-[#F4F4F4]' : 'text-[#4D4D4D]',
                                ].join(' ')}
                            />
                        }
                    />
