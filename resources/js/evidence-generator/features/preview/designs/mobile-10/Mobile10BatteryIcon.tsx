...
import { mobile10FontFamily } from './mobile10Colors';
...
export function getMobile10BatteryProgressWidth(level: number): number {
    const value = Math.max(0, Math.min(100, Math.round(level)));

    return INNER_WIDTH * (value / 100);
}

export type Mobile10BatteryIconProps = {
    level: number;
...
export function Mobile10BatteryIcon({ level, themeMode, progressColor, foregroundColor }: Mobile10BatteryIconProps) {
    const value = Math.max(0, Math.min(100, Math.round(level)));
    const clipId = `Mobile10-battery-${useId().replace(/:/g, '')}`;
    const progressWidth = getMobile10BatteryProgressWidth(level);
