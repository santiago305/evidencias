...
export function DayChip({ text, themeMode = 'light' }: { text: string; themeMode?: PreviewThemeMode }) {
    ...
    return (
        <div className="sticky top-[2.5px] z-20 my-[5px] flex justify-center">
            <span
                className={[
                    'rounded-[5px] px-2.5 py-[2.5px] text-[12.5px] !font-normal shadow',
                    colors ? '' : isDark ? 'bg-[#12181C] text-[#767C80]' : 'bg-[#fefdfc] text-[#1b1b1c]',
                ]
                    .filter(Boolean)
                    .join(' ')}
...
import type { WhatsappMessageStatus } from './whatsappTypes';
...
export function EncryptedMessage({ themeMode = 'light' }: { themeMode?: PreviewThemeMode }) {
    const isDark = themeMode === 'dark';
    const encryptedTextSize = 'text-[11px] leading-[14px]';
...
export function TempporalMessage({ themeMode = 'light' }: { themeMode?: PreviewThemeMode }) {
    ...
    const horizontalPadding = 'px-10';
    const maxWidth = 'max-w-none';
    const messageTextSize = 'text-[11px] leading-[14px]';
...
export function DesactiveTemporalMessage({ themeMode = 'light' }: { themeMode?: PreviewThemeMode }) {
    ...
    return (
        <div className="relative">
            <div className="relative pb-2.5 px-3">
                <div className="flex justify-center">
                    <div className="mx-auto flex w-fit max-w-full flex-col items-center justify-center">
                        <div
                            className={[
                                'relative mb-0 box-border inline-flex w-fit max-w-full flex-none items-center rounded-[9.1875px] px-[15px] pt-[5px] pb-[6.25px] text-center text-[12.5px] leading-[15px] shadow-[0_1px_0.5px_rgba(11,20,26,0.13)]',
                                isDark ? 'bg-[#12181C] text-[#767C80]' : 'bg-[#fefdfc] text-[#667781]',
                            ].join(' ')}
                        >
                            <div className="relative z-10 rounded-[9.1875px]">
