    ...
    return (
      <div className="mt-1 flex w-full flex-nowrap items-center justify-center gap-2 self-stretch">
          {visibleActions.map((action) => (
              <button key={action.key} type="button" className={actionButtonClassName}>
                  <span className="text-[#00a884]">{action.icon}</span>
                  <span className="segoe-ui truncate">{action.label}</span>
              </button>
          ))}
      </div>
  );
