...
export function getMobile12BatteryProgressWidth(level: number): number {
    const value = Math.max(0, Math.min(100, Math.round(level)));

    return INNER_WIDTH * (value / 100);
}

export type Mobile12BatteryIconProps = {
    level: number;
...
export function Mobile12BatteryIcon({ level, themeMode, progressColor, foregroundColor }: Mobile12BatteryIconProps) {
    const value = Math.max(0, Math.min(100, Math.round(level)));
    const clipId = `Mobile12-battery-${useId().replace(/:/g, '')}`;
    const progressWidth = getMobile12BatteryProgressWidth(level);
