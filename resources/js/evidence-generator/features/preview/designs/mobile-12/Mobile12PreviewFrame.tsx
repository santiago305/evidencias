...
import { Mobile12PreviewFooter } from './Mobile12PreviewFooter';
import { Mobile12PreviewHeader } from './Mobile12PreviewHeader';
import './mobile12.css';

type Mobile12PreviewFrameProps = {
    children: ReactNode;
...
export function Mobile12PreviewFrame({
    children,
    themeMode,
    contentClassName = '',
    ...
    return (
        <div className={['flex h-full w-full items-center justify-center p-5', isDark ? 'bg-slate-950' : 'bg-slate-100'].join(' ')}>
            <div
                id="CAPTURA"
                className={[
                    'mobile12-font flex h-[950px] max-h-[calc(100vh-2.5rem)] w-[470.5px] max-w-full flex-col overflow-hidden shadow-2xl',
                    isDark ? 'bg-[#101418] shadow-black/45' : 'bg-white shadow-slate-300/60',
                ].join(' ')}
                style={{ width: frame?.width, height: frame?.height }}
            >
                {!hideSystemHeader ? (
                    <Mobile12PreviewHeader
                        themeMode={themeMode}
