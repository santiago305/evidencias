...
const contactadoReplyProfileFallbacks: Record<
  TipoCliente,
  ContactadoReplyProfile[]
> = {
...
function pickContactadoAdvisorReply(
  ...
  for (const profile of contactadoReplyProfileFallbacks[tipoCliente]) {
    const options =
      contactadoRespuestasAsesorPorIntencionData[intent]?.[profile];
...
function pickInformativoAdvisorReply(
  intent: ConversationIntent,
  tipoCliente: TipoCliente,
  rng: () => number
) {
...
export function buildWhatsappConversation(
  data: WhatsappData,
  messageStatus?: MsgStatus
) {
  const seed = Math.floor(
    (Date.now() + Math.random() * 1_000_000) % 1_000_000_000
  );
  ...
  const normalizeReply = (lines: string[]) =>
    lines.map((line) => interpolateTemplate(line, extendedTemplateValues));
  const normalizeReplyVariant = (reply: string | string[]) =>
    normalizeReply(Array.isArray(reply) ? reply : [reply]);

  const modeConfig = getWhatsappConversationModeConfig(
    data.modoEntrada,
    data.tipoCliente
  );

  const firstMessageLines = normalizeReply(
    pick(modeConfig.mensajeEntrada, rng)
  );
  ...
  const pickedInitialResponse =
    modeConfig.respuestaClienteInicial.length > 0
      ? pick(modeConfig.respuestaClienteInicial, rng)
      : null;
  const pickedReplyVariant = pickedInitialResponse
    ? pick(
        pickedInitialResponse.respuestas,
        rng
      )
  ...
  const activeIntent =
    (data.modoEntrada === "contactado" || data.modoEntrada === "informativo") &&
    isConversationIntent(pickedInitialResponse?.tipo)
  ...
  const secondExchangePlan =
    ...
    activeIntent !== null
      ...
      : null;
  const shouldUseHumanFarewell =
    (data.modoEntrada === "contactado" || data.modoEntrada === "informativo") &&
    activeIntent !== "desinteresado";
  const despedidaClienteLines = shouldUseHumanFarewell
    ? normalizeReplyVariant(pick(respuestasClienteDespedidaData, rng))
    : null;
  const despedidaAsesorLines = shouldUseHumanFarewell
    ? normalizeReplyVariant(pick(respuestasAsesorDespedidaData, rng))
    : null;
  ...
  const despedidaClienteTime = secondExchangePlan
    ? formatTimeShort(t7)
    : formatTimeShort(t5);
  const despedidaAsesorTime = secondExchangePlan
    ? formatTimeShort(t8)
    : formatTimeShort(t6);
